...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.documentElement.style.setProperty('--background-color', bodyBgColor);
    
    const inputContainer = document.getElementById('input-container');
    const bgLayer = document.querySelector('.input-background-layer');
    
    if (inputContainer && bgLayer) {
        // 监听窗口大小变化，调整背景层
        window.addEventListener('resize', adjustBackgroundLayer);
        
        // 初始调整
        adjustBackgroundLayer();
        ...
        const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                if (mutation.attributeName === 'class' || 
                    ...
                    // 当主题变化时，更新背景色变量
                    const newBgColor = getComputedStyle(document.body).backgroundColor;
    ...
    function adjustBackgroundLayer() {
        if (!bgLayer) return;
        
        // 移动设备上使背景层更高
        if (window.innerWidth <= 576) {
            bgLayer.style.height = '180px';
        } else if (window.innerWidth <= 768) {
            bgLayer.style.height = '140px';
        } else {
            bgLayer.style.height = '120px';
        }
    }
});
